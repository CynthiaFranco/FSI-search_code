...
class RamificationQueue(Queue):
    """Utiliza una cola para devolver el camino resultante teniendo en cuenta los costes de los nodos."""
    def __init__(self):
        self.A = []
    ...
    def extend(self,items):
        self.A.extend(items)
        list.sort( self.A, key=lambda f: f.path_cost)

    def pop(self):
        return self.A.pop(0)

class HeuristicQueue(Queue):
    """Devuelve el camnio pero esta vez, se tendran en cuenta los costes de los nodos y
    la suma heuristica del camino desde el nodo inicial al final."""
    ...
    def extend(self,items):
        self.A.extend(items)
        list.sort( self.A, key=lambda f: f.path_cost + self.problem.h(f))
